...
namespace GaussianSplatting.Runtime
{
    class GaussianSplatRenderSystem
    {
        ...
        void OnPreCullCamera(Camera cam)
        {
            ...
            int useSortFree = 0;
            float sortFreeBackgroundWeight = 0.0f;
            foreach (var kvp in m_ActiveSplats)
            {
                var gs = kvp.Item1;
                if (gs.asset != null && gs.asset.isSortFree)
                {
                    useSortFree = 1;
                    sortFreeBackgroundWeight = gs.asset.sortFreeBackgroundWeight;
                    break;
                }
            }
            m_CommandBuffer.SetGlobalInt(
                GaussianSplatRenderer.Props.UseSortFree,
                useSortFree
            );
            m_CommandBuffer.SetGlobalFloat(
                GaussianSplatRenderer.Props.SortFreeBackgroundWeight,
                sortFreeBackgroundWeight
            );
